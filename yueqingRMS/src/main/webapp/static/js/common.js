...
(function($) {
	...
	$.dialog = function(options) {
		...
		if (settings.ok != null) {
			$dialogOk = $('<input type="button" class="button_x" value="' + settings.ok + '" \/>').appendTo($dialogBottom);
		}
		if (settings.cancel != null) {
			$dialogCancel = $('<input type="button" class="button_x" value="' + settings.cancel + '" \/>').appendTo($dialogBottom);
		}
		if (!window.XMLHttpRequest) {
	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: cebcm.base + "/admin/file/browser.jhtml",
				uploadUrl: cebcm.base + "/admin/file/upload.jhtml",
				callback: null
			};
			$.extend(settings, options);
			...
			return this.each(function() {
				...
				$browserButton.click(function() {
					...
					$browserParentButton = $('<a href="javascript:;" class="button_x">' + message("admin.browser.parent") + '<\/a>').appendTo($browserBar);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = cebcm.base + "/resources/admin/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = cebcm.base + "/resources/admin/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
